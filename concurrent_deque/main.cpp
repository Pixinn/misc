...
int main(int, char **)
{
    TDequeConcurrent< string > deque;
    constexpr unsigned NB_MESSAGE = 1000u;

    //Consuming thread
    thread consumer([&deque, NB_MESSAGE]
    {
        auto i = 0u;
        while (i<NB_MESSAGE)  {
            auto message = deque.pop_front();     //blocks until a message is in the list
    ...
    for (auto i = 0u; i < NB_MESSAGE; ++i){
        deque.emplace_back("Message from main thread: " + to_string(i) );
    }
